...
def main(file_name):
    ...
    for boule in boules:
        ...
        for card in cards:
            ...
            if card.won is False:
                card.handle_number(boule)

                # On valide si on gagne
                if card.has_bingo:
                    card.won = True

                    # On valide le nombre de cartes restantes
                    remaining_cards = len(cards) - sum([1 if c.won is True else 0 for c in cards])
                    print(f"Il reste {remaining_cards} cartes...")
                    if remaining_cards == 0:
                        # La dernière a gagné, on va prendre son score
                        print("Score de la dernière carte")
